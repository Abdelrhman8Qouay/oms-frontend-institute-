<template>
    <div class="min-h-screen bg-gray-100">
            ...
            <div class="grid grid-cols-12 gap-6">
                        ...
                        <div class="flex space-x-4 overflow-x-auto">
                            <button v-for="category in categories" :key="category.id" :class="[
                                ...
                                currentCategory === category.id
                                    ? 'bg-blue-600 text-white'
                                    : 'bg-gray-200 text-gray-700'
                            ]" @click="fetchCategoryItems(category.id)">
                                {{ category.name }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-4 rounded-lg shadow">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div v-for="item in categoryItems" :key="item.id"
                                class="p-4 border rounded-lg cursor-pointer hover:bg-gray-50" @click="addItem(item)">
                ...
                <div class="col-span-4">
                    <div class="bg-white p-4 rounded-lg shadow sticky top-4">
                        ...
                        <button v-if="orderType === 'delivery'" class="w-full mb-4 px-4 py-2 bg-gray-200 rounded-md"
                            @click="showCustomerModal = true">
                            {{ customerDetails ? 'Update Customer Details' : 'Add Customer Details' }}
                        </button>
                        ...
                        <!-- Submit Button -->
                        <button class="w-full mt-4 px-4 py-2 bg-blue-600 text-white rounded-md disabled:bg-gray-400"
        ...
        <CustomerDetailsModal v-if="showCustomerModal" :initial-details="customerDetails"
            @close="showCustomerModal = false" @submit="handleCustomerDetails" />
    </div>
</template>
...
onMounted(async () => {
    await fetchCategories('default-menu-id') // Replace with actual menu ID
})
